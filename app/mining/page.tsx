...
export default function MiningPage() {
  ...
  useEffect(() => {
    fetch('/api/packages?industry=enterprise').then(r => r.json()).then(d => setApiPackages(d.packages || [])).catch(() => {});
  }, []);
      ...
      <section className="py-24 bg-sm-card/20">
        <div className="max-w-7xl mx-auto px-6">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {PACKAGES.map((pkg, i) => {
              const tierMap: Record<string, string> = { 'Starter': 'base', 'Growth': 'growth', 'Pro': 'ai-systems' };
              const apiPkg = apiPackages.find(a => a.tier === (tierMap[pkg.tier] || pkg.tier.toLowerCase()));
